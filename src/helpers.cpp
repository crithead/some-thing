...
void GetOptions(int argc, const char *const *argv, Dictionary& opts)
{
    ...
    int opt_index = 0;
    while (true) {
        int opt = getopt_long(argc, const_cast<char *const *>(argv), OPTIONS,
                              LONG_OPTIONS, &opt_index);
        std::cout << "opt = " << opt << ", " << static_cast<char>(opt)
                  << std::endl;
        if (opt == -1) {
            break;
        }
...
void PrintUsage(void)
{
    ...
    std::cout
            << "  -l <level>,     Set the log level (ERROR, WARNING, NOTICE," " INFO, DEBUG)"
            << std::endl;
